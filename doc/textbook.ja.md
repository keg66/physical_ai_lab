[English](textbook.md) | 日本語

# Physical AI Hands-on Textbook

> [!NOTE]
> 本文書が原文です。英語版はDay 1〜5の構成と節番号を維持した要約翻訳です。両者に差異がある場合は、日本語版を正として扱います。
...
本教材の本文は、筆者がChatGPTとの対話を通じて学習・実験した履歴をもとに、ChatGPTによって生成されました。筆者は公開に向けて内容を確認し、構成や表現を編集しています。

一方、このリポジトリのコードは、AIによるコード補完や単純なロジック生成の支援を一部利用しながら、ほぼすべて筆者自身が作成しました。掲載しているコードの内容は、筆者が一通り確認しています。

AIが生成した文章には、不正確または不完全な説明が含まれる可能性があります。誤りや改善点を見つけた場合は、IssueまたはPull Requestでお知らせください。掲載内容の選択、編集および公開については、リポジトリ所有者が責任を持ちます。
...
本文は、学習時に行った実験から得た設計上の知見を再構成したものです。会話履歴を知らなくても読めるよう、各Dayの冒頭に関連コードを示します。環境構築、APIキー、再現用sceneの生成、実行コマンドは[リポジトリのREADME](../README.ja.md)を参照してください。
...
実行コード：[`day2/rt_mini_lab.py`](../day2/rt_mini_lab.py)。コマンドは[README](../README.ja.md#2-外部apiなしのmini-lab)を参照してください。
...
BCのmini-labはCPUでも実行できます。LeRobotの例は外部datasetを取得し、ACTのtraining stepにはCUDA環境が必要です。実行条件は[README](../README.ja.md#4-lerobot--act)を参照してください。
...
実行にはTypeSafe AIから発行されたcredentialが必要で、`RecoveryContext`に含まれる情報が外部APIへ送信されます。利用前にproviderの最新の利用条件とデータ取扱方針を確認してください。設定方法は[README](../README.ja.md#5-jevによるsystem-1-recovery)を参照してください。
...
以下は実装の方向性を含む全体像です。現在のリポジトリでは、ROS 2、Gazebo、実機、実際のclassical/learned skill backend、runtime/control safetyは未実装です。実装状況の一覧は[README](../README.ja.md#実装範囲)を参照してください。
